 ...
 *
 * Each run_*() function corresponds to one class defined in cnn_models.py:
 *
 *   run_cnn_2d_v1()               → CNN_2D_v1   (PYA: 2D-CNN, 2 conv + 2 FC)
 *   run_cnn_2d_v2()               → CNN_2D_v2   (PYB: 2D-CNN, parallel branches)
 *   run_cnn_2d_v3()               → CNN_2D_v3   (PYC: 2D-CNN, 1 extra branch)
 *   run_cnn_1d_v1()               → CNN_1D_v1   (PYD: 1D-CNN, 1 conv + 1 FC)
 *   run_cnn_1d_v2()               → CNN_1D_v2   (PYE: 1D-CNN, 2 conv + 1 FC)
 *   run_cnn_1d_v3()               → CNN_1D_v3   (PYF: 1D-CNN, 1 conv + 2 FC)
 *   run_mobilenet_v3_custom()     → MobileNetV3Custom  (PYG: MobileNetV3-small
 *                                    width_mult=0.35, 1-channel input, SE blocks)
 ...
 *
 * All functions create deterministic dummy inputs and weights.  Dropout is a
 * no-op at inference time and is omitted entirely.  BatchNorm is applied in
 * inference mode (frozen mean/var from the dummy BatchNormLayer).
 *
 * The helper apply_conv_bn_act() encapsulates the most common fused pattern:
 *     Conv2D → BatchNorm → Activation
 * which is the most important fusion target for CGRA implementations.
 */
